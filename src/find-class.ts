...
export function checkTree(text, obTree, line): boolean {
    ...
    if (typeof isClosed.closed != "boolean") {
        obTree.waitOpenTag = isClosed.closed.slice(0);
    }
    if (typeof isClosed.opened != "boolean") {
        if (obTree.waitOpenTag.length == 0) {
            return true;
        }
        if (obTree.waitOpenTag.slice(-1)[0] == isClosed.opened.slice()[0]) {
            obTree.waitOpenTag.shift();
        }
    }
...
function is_closed(text): any {
    ...
    let match_closed = text.match(/<(\w+) .*>.*<\/(\w)>/),
        ...
        match_close = text.match(/<\/(\w+)>$/),
        match_opened = text.match(/<(\w+) .*>$/);
